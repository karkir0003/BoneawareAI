...
class MURADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        absolute_root = os.path.abspath(self.root_dir)

        parts = self.root_dir.split("/")
        start_part = "/".join(parts[-2:])

        relative_img_path = os.path.relpath(img_path, start=start_part)
        full_img_path = os.path.join(absolute_root, relative_img_path)
        ...
        # Fetch the label
        label = self.label_map.get(img_dir_key, -1)
        if label == -1:
            raise KeyError(f"Label not found for study path: {img_dir_key}")
